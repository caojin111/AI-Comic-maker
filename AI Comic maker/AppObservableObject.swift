...
class AppObservableObject: ObservableObject {
                ...
                if httpResponse.statusCode == 200 {
                    ...
                    do {
                        ...
                        for (index, page) in decoded.pages.enumerated() {
                            let hasPrompt = (page.imagePrompt ?? "").isEmpty ? "no" : "yes"
    ...
    /// 同时存入 preloadedImages（当前会话）、ImageCache（磁盘缓存）和 Documents（持久化）
    /// 返回保存的本地文件名（用于更新 StoryPage.localImagePath）
    @discardableResult
    func downloadAndCacheImage(pageId: UUID, imageUrl: String) async -> UIImage? {
        ...
        // 持久化保存到 Documents/comic_images/（不会被系统清除）
        let filename = "\(pageId.uuidString).jpg"
        if let data = image.jpegData(compressionQuality: 0.92) {
            ImageCache.shared.savePersistent(data: data, filename: filename)
            // 更新 pages 中对应页的 localImagePath
            await MainActor.run {
                if let idx = pages.firstIndex(where: { $0.id == pageId }) {
                    pages[idx].localImagePath = filename
                }
            }
        }
        ...
        print("[AppObservableObject] 已缓存图片到内存、ImageCache 和 Documents，pageId：\(pageId)")
        return image
    }
